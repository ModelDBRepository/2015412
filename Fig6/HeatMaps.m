...
% Create text files for correspondence of GLTRP and Temperature to j and i 

% Temperature
[fidTemp,message]= fopen('T.txt', 'w');
for i=1:NC2 %T
    fprintf(fidTemp,'%f  ',i,Tv(i));% file for correspondence 
    fprintf(fidTemp,'\n');
    Tvalues(i) = {num2str(Tv(i))};% label values for heat map
end
fclose(fidTemp);
...
for j=1:NC1%GleakTest
    ...
    GLTestValues(j) = {num2str(GLTestv(j))};%label values for heat map
 end
fclose(fidGleakTest);
...
colorbarString='MF (Hz)';
 ...
 figure;
    for ii=1:length(pp2range)
        ...
        'MarkerFaceAlpha',.6, 'MarkerEdgeAlpha',1);
        hold on
    end
    colormap(jet); 
    ...
    clim=get(c, 'limits'); %get current limits
            hold on
               
                ax = axes;
                plot(ax,G_LTest,TK-273.15,'-','color',[0.4 0.4 0.4],'LineWidth',2);%[0.4 0.4 0.4],'LineWidth',3)
                axis([0. 1. 4. 24.]);
                set(gca, 'FontSize', 14,'FontWeight', 'bold');
                ax.Color = 'none';
                ax.XTick = [];
                ...
                %set(gca,'box','off');
                axis off;  
                % Adjust axes position after colorbar rescale
